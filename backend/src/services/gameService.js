...
class GameService {
  constructor() {
    ...
    this.gameState.probes = [this.createProbe(0)];
  ...
  createProbe(index) {
    ...
    // Distribute probes across multiple orbit layers if crowded
    // Layer capacity: 6 probes per orbit for good spacing
    const probesPerOrbit = 6;
    const orbitLayer = Math.floor(index / probesPerOrbit); // 0, 1, 2...
    const positionInOrbit = index % probesPerOrbit;
    const probesInThisOrbit = Math.min(
      probesPerOrbit,
      totalProbes - orbitLayer * probesPerOrbit
    );

    // Even spacing within this orbit layer
    const angleSpacing = (Math.PI * 2) / probesInThisOrbit;
    const orbitPosition = positionInOrbit * angleSpacing + Math.PI; // Offset from satellites

    return {
      id: `probe-${Date.now()}-${index}`,
      orbitPosition: orbitPosition,
      orbitLayer: orbitLayer, // 0 = innermost probe orbit, 1 = next layer, etc.
  ...
  // Recalculate all probe positions to ensure even spacing
  recalculateProbePositions() {
    const totalProbes = this.gameState.probes.length;
    const probesPerOrbit = 6;

    this.gameState.probes.forEach((probe, index) => {
      const orbitLayer = Math.floor(index / probesPerOrbit);
      const positionInOrbit = index % probesPerOrbit;
      const probesInThisOrbit = Math.min(
        probesPerOrbit,
        totalProbes - orbitLayer * probesPerOrbit
      ...
      const angleSpacing = (Math.PI * 2) / probesInThisOrbit;
      probe.orbitPosition = positionInOrbit * angleSpacing + Math.PI;
      probe.orbitLayer = orbitLayer;
    });
  }
  ...
  async generateEvents() {
    ...
    try {
        ...
        if (riskLevel !== "safe") {
          events.push({
            ...
            description: `Asteroid ${
              asteroid.name
            } is approaching Earth. Diameter: ${Math.round(
              asteroid.diameter
            )}m, Velocity: ${Math.round(
              asteroid.velocity
            )} km/s, Miss Distance: ${Math.round(
              asteroid.missDistance / 1000
            )}k km`,
            data: asteroid,
      ...
      this.gameState.events = [...events, ...this.gameState.events].slice(
        0,
        50
      ); // Keep last 50 events
      this.gameState.threats = events.filter(
        (e) => e.type === "asteroid_detected"
      );
      ...
      this.gameState.threats = simulatedEvents.filter(
        (e) => e.type === "asteroid_detected"
      );
    ...
    for (let i = 0; i < numAsteroids; i++) {
      ...
      if (riskLevel !== "safe") {
        events.push({
          ...
          description: `INCOMING! ETA: ${gameMinutes.toFixed(
            1
          )} min | ${Math.round(diameter)}m | ${Math.round(
            velocity
          )} km/s | ${Math.round(distance / 1000)}k km`,
          data: asteroid,
          timestamp: new Date().toISOString(),
          requiresAction: true,
        });
  ...
  isAsteroidDetected(asteroid) {
    ...
    if (
      !asteroid.data.approachAngle &&
      !asteroid.data.polarAngle &&
      !asteroid.data.timeToImpact
    ) {
      return true; // Real NASA data - always show
    }

    const elapsedMinutes = (this.gameState.day - 1) * 1440; // Minutes since game start
    const asteroidPos = this.calculateAsteroidPosition3D(
      asteroid,
      elapsedMinutes
    );
  ...
  generateRandomEvent() {
    ...
    const eventTemplates = {
      solar_flare: {
        ...
        description:
          "A solar flare is disrupting satellite communications. Radar coverage reduced temporarily.",
        requiresAction: true,
      },
      satellite_malfunction: {
        title: "Satellite Malfunction",
        description:
          "One of our monitoring satellites has malfunctioned. Detection capabilities reduced.",
        requiresAction: true,
      },
      unknown_object: {
        title: "Unknown Object Detected",
        description:
          "An unidentified object has been detected in Earth's vicinity. Requires investigation.",
        requiresAction: true,
      },
      communication_blackout: {
        title: "Communication Blackout",
        description:
          "Solar interference is causing communication issues with deep space probes.",
  ...
  processAction(action) {
    ...
    switch (type) {
      case "deploy_satellite":
        ...
        const newSatellite = this.createSatellite(
          this.gameState.satellites.length
        );
      ...
      case "launch_probe":
        ...
        // Recalculate all probe positions for even spacing
        this.recalculateProbePositions();
      ...
      case "deflect_asteroid":
        ...
        const avgProbeLevel =
          this.gameState.probes.length > 0
            ? this.gameState.probes.reduce((sum, p) => sum + p.level, 0) /
              this.gameState.probes.length
            : 1;
        const avgProbePower =
          this.gameState.probes.length > 0
            ? this.gameState.probes.reduce(
                (sum, p) => sum + (p.laserPower || 100),
                0
              ) / this.gameState.probes.length
            : 100;
        ...
        successChance = Math.min(
          0.95,
          successChance + (avgProbeLevel - 1) * 0.1
        ); // +10% per avg probe level
        ...
        if (success) {
          if (willFragment) {
            ...
            for (let i = 0; i < fragmentCount; i++) {
              const fragmentDiameter =
                asteroidDiameter / (fragmentCount + Math.random());
              const fragmentThreat = {
                ...
                data: {
                  ...
                  approachAngle:
                    threat.data.approachAngle + (Math.random() - 0.5) * 0.5, // Scatter angle
            ...
            this.gameState.threats = this.gameState.threats.filter(
              (t) => t.id !== targetId
            );
      ...
      case "upgrade_satellite":
        const satellite = this.gameState.satellites.find(
          (s) => s.id === targetId
        );
        if (!satellite) {
        ...
        result.success = true;
        result.message = `Satellite upgraded to Level ${
          satellite.level
        }! Detection radius increased to ${satellite.detectionRadius.toFixed(
          1
        )} units.`;
        result.scoreChange = 100;
        break;
      ...
      case "asteroid_impact":
        ...
        const impactThreat = this.gameState.threats.find(
          (t) => t.id === targetId
        );
        if (!impactThreat) {
        ...
        if (impactThreat.severity === "critical" || impactDiameter > 500) {
          ...
          impactThreat.severity === "moderate" ||
          impactDiameter > 200
        ) {
        ...
        // Cap earth health
        this.gameState.earthHealth = Math.max(
          0,
          Math.min(100, this.gameState.earthHealth)
        );
        ...
        result.success = true;
        result.message = `${
          impactThreat.data?.name || "Asteroid"
        } impacted Earth! -${damage}% health`;
        ...
        if (this.gameState.earthHealth <= 0) {
          ...
          result.message =
            "Earth's health has reached critical levels. Mission failed.";
        }
        break;
  ...
  async advanceDay() {
      ...
      burnedUpAsteroids.forEach((asteroid) => {
        this.gameState.events.push({
          ...
          description: `${asteroid.name} (${
            asteroid.diameter
          }m) safely burned up at ${asteroid.altitude.toFixed(1)}km altitude`,
          timestamp: new Date().toISOString(),
          icon: "🔥",
        });
      });
    ...
    const criticalThreats = this.gameState.threats.filter(
      (t) => t.severity === "critical"
    );
    const moderateThreats = this.gameState.threats.filter(
      (t) => t.severity === "moderate"
    );
    ...
    if (criticalThreats.length === 0 && moderateThreats.length === 0) {
      this.gameState.earthHealth = Math.min(
        100,
        this.gameState.earthHealth + 2
      );
    }
